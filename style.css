...
function resetGame() {
    ...
    thingie.style.top = l1 + step + "px"; // Reset the thingie position
    thingie.style.left = 0 + "px"; // Reset the position
    startTimer(); // Restart the timer
}

// Check for win condition (when the player reaches the home)
...
// Modify the existing movement functions to include the win check:
...
function winGame() {
    clearInterval(timerInterval);
    gameMessage.style.display = "block";
    gameMessage.textContent = "You Won!";  // Display "You Won!"

    // Apply the 'won' class for green background
...
function gameOver(status) {
    ...
    if (status === "lost") {
        ...
        gameMessage.classList.add("lost");  // Apply red background for losing
    }
